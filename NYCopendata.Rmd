...
live_streets_sf <- live_streets %>% 
  separate(geometry, sep = ":", into = c("piece", "piece2")) %>% 
  mutate(piece3 = str_sub(piece2, start = 1, end = -8)) %>% 
  mutate(geometry = map(
    paste0('{"type": "LineString","coordinates":', piece3, "}" ),
    ~fromJSON(as.character(.x)))) %>% 
  #mutate(geometry = geojson_sf(piece3))
  st_as_sf(geometry)

  
live_streets_sf$piece3 %>% 
map_dfr(~geojson_sf(.x))

geojson_sf(live_streets_sf$piece3[1])



test <- paste0('{"type": "LineString","coordinates":', live_streets_sf$piece3[1], "}" )

geojson_sf(test)

live_streets %>% 
  separate(geometry, sep = ":", into = "piece")

sf <- geojson_sf("https://api.qri.cloud/ds/get/chriswhong/nyc-open-streets/body.csv")
